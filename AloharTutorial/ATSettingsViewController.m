...
- (void)viewDidLoad
{
    ...
    self.tokenLabel.text = [defaults stringForKey:@"ALAPIToken"];
    self.userIDLabel.text = [defaults stringForKey:@"ALAPIUserID"];
    self.uidLabel.text = [defaults stringForKey:@"ALAPIUID"];
    if ([Alohar monitoringUser]){
        [self.moniteringUserSwitch setOn:YES animated:YES];
    } else {
        [self.moniteringUserSwitch setOn:NO animated:YES];
    }
}
